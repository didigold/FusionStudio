...
import {
  Brain, Settings, Play, Square, History, FolderOpen,
  Database, Layers, TrendingDown, RefreshCw, Zap, Trash2,
  Plus, X, ChevronDown, ChevronRight, Lightbulb, Info,
  CheckCircle2, Circle, Loader2, Cpu, Sparkles, Shield,
  BookOpen, AlertTriangle, BarChart3, Target, Check, GripVertical
} from 'lucide-react'
...
const PRESETS = [
  { id: 'fast', label: 'Fast Prototype', icon: Zap, epochs: 20, lr: 0.01, patience: 5, desc: 'Quick sanity check on small data' },
  { id: 'balanced', label: 'Balanced', icon: Target, epochs: 100, lr: 0.001, patience: 15, desc: 'Standard production training' },
  { id: 'deep', label: 'Deep Training', icon: Sparkles, epochs: 300, lr: 0.0001, patience: 30, desc: 'Maximum accuracy, large datasets' },
] as const
...
const BEST_PRACTICES = [
  {
    icon: BarChart3, title: 'Data Balance',
  ...
  {
    icon: Shield, title: 'Overfitting Prevention',
  ...
  {
    icon: Database, title: 'Data Diversity',
  ...
  {
    icon: TrendingDown, title: 'Epoch Count',
  ...
  {
    icon: Sparkles, title: 'Learning Rate',
  ...
  {
    icon: AlertTriangle, title: 'Video + MF4 Alignment',
  ...
  {
    icon: Target, title: 'Minimum Data',
          ...
          <div key={phase} className="flex items-center gap-2 flex-1">
            ...
            {i < PHASES.length - 1 && (
              <ChevronRight className={cn(
                "w-4 h-4 shrink-0",
                isComplete ? "text-emerald-400/50" : "text-muted-foreground/20"
              )} />
            )}
          </div>
        )
...
export default function HuMindTab() {
  ...
  useEffect(() => {
    const update = () => setTipText(getContextualTip(brainPhase, brainEpochData))
    update()
  ...
  const bestEpoch = useMemo(() => {
    ...
    brainEpochData.forEach((d: any, i: number) => {
      ...
      if (acc > bestAcc) { bestAcc = acc; bestIdx = i }
    })
    return bestIdx
  }, [brainEpochData])
  ...
  const handleAddProject = async () => {
    ...
    setScanning(true)
    try {
      const res = await fetch(`/api/brain/projects?root=${encodeURIComponent(path)}`)
      ...
      if (projects.length > 0) {
        ...
        setSelectedProjects(prev => [...prev, ...projects.map((p: any) => p.path)])
        addLog(`Added project folder: ${name} with ${projects.length} participant(s)`)
      } else {
  ...
  const toggleGroupExpanded = (groupPath: string) => {
    setProjectGroups(prev =>
      prev.map(g => g.path === groupPath ? { ...g, isExpanded: !g.isExpanded } : g)
    )
  }
  ...
  const handleStartTraining = async () => {
    ...
    } catch (err) { setBrainTraining(false); addLog(`Error starting training: ${err}`) }
  }
  ...
  const handleLoadModelConfig = (model: any) => {
    if (model.metadata) {
      ...
      addLog(`Loaded configuration parameters from saved model variant: ${model.variant || model.architecture}`)
    } else {
  ...
  const modelTrainedPaths = useMemo(() => {
    ...
    // Also from history
    if (brainHistory?.mlp?.projects) brainHistory.mlp.projects.forEach((p: string) => paths.add(p))
    if (brainHistory?.multimodal?.projects) brainHistory.multimodal.projects.forEach((p: string) => paths.add(p))
    return paths
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 relative">
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/5 bg-surface-2/50 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Distraction Prediction Engine</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-500/5 border border-amber-500/10 rounded-lg max-w-md">
            ...
            <span className="text-xs text-amber-300/90 leading-tight truncate">{tipText}</span>
          </div>
      ...
      <div className="flex-1 flex min-h-0 w-full relative">
        ...
        <div style={{ width: sidebarWidth }} className="shrink-0 flex flex-col min-h-0 bg-surface-1/40 h-full border-r border-white/5 relative">
          <ScrollArea className="flex-1">
            <Accordion type="multiple" defaultValue={["config", "projects"]} className="w-full">
              ...
              <AccordionItem value="config" className="border-b border-white/5">
                ...
                <AccordionContent className="px-4 pb-4 pt-1 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-muted-foreground/80 uppercase tracking-wide">Architecture</label>
                  ...
                  {/* Presets */}
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-muted-foreground/80 uppercase tracking-wide">Training Presets</label>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-1">
                    <div className="space-y-1 col-span-2">
                      <label className="text-xs font-bold text-muted-foreground/80 uppercase tracking-wide">Model Name</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-muted-foreground/80 uppercase tracking-wide">Epochs</label>
                      <input
                        type="number"
                        value={brainEpochs}
                        onChange={e => { setBrainEpochs(parseInt(e.target.value) || 100); setActivePreset(null) }}
                        className="w-full bg-surface-3 border border-border/50 rounded-lg px-3 py-2 text-xs text-foreground focus:outline-none focus:border-primary/30 transition-colors"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-muted-foreground/80 uppercase tracking-wide">Learning Rate</label>
                      <input
                        type="text"
                        value={brainLR}
                        onChange={e => { setBrainLR(parseFloat(e.target.value) || 0.001); setActivePreset(null) }}
                        className="w-full bg-surface-3 border border-border/50 rounded-lg px-3 py-2 text-xs text-foreground focus:outline-none focus:border-primary/30 transition-colors"
                      />
                    </div>
                    {brainArchitecture === 'multimodal' && (
                      <div className="space-y-1 col-span-2">
                        <label className="text-xs font-bold text-muted-foreground/80 uppercase tracking-wide">Early Stopping Patience</label>
                        <input
                          type="number"
                          value={brainPatience}
                          onChange={e => { setBrainPatience(parseInt(e.target.value) || 15); setActivePreset(null) }}
                          className="w-full bg-surface-3 border border-border/50 rounded-lg px-3 py-2 text-xs text-foreground focus:outline-none focus:border-primary/30 transition-colors"
                        />
                      </div>
                    )}
                ...
                <AccordionContent className="px-4 pb-4 pt-1 space-y-3">
                  ...
                  {projectGroups.length > 0 ? (
                    <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                      {projectGroups.map(group => {
                        ...
                        const selectedInGroup = group.participants ? group.participants.filter(p => selectedProjects.includes(p.path)).length : 0
                        const allSelected = selectedInGroup === totalParticipants
                        const noneSelected = selectedInGroup === 0
                        const radioValue = allSelected ? "all" : noneSelected ? "none" : "mixed"

                        return (
                          <div key={group.path} className="border border-white/5 bg-surface-3/15 rounded-xl overflow-hidden">
                            {/* Folder header */}
                            <div className="flex items-center justify-between px-3 py-2 bg-surface-3/30 border-b border-white/5">
                              ...
                              >
                                ...
                                <span className="text-xs font-bold text-foreground truncate" title={group.path}>
                                  {group.name}
                                </span>
                              </div>
                              <button
                            ...
                            <div className="p-2.5 space-y-2">
                              ...
                              <div className="flex items-center justify-between pb-1.5 border-b border-white/[0.03]">
                                <span className="text-xs text-muted-foreground/70 font-semibold">Include:</span>
                                <RadioGroup
                                  value={radioValue}
                                  onValueChange={val => handleGroupRadioChange(group.path, val)}
                                  className="flex items-center gap-3"
                                >
                                  <div className="flex items-center gap-1">
                                    <RadioGroupItem value="all" id={`all-${group.path}`} className="h-3.5 w-3.5 border-white/20" />
                                    <Label htmlFor={`all-${group.path}`} className="text-xs font-semibold cursor-pointer text-foreground/80">
                                      All
                                    </Label>
                                  </div>
                                  <div className="flex items-center gap-1">
                                    <RadioGroupItem value="none" id={`none-${group.path}`} className="h-3.5 w-3.5 border-white/20" />
                                    <Label htmlFor={`none-${group.path}`} className="text-xs font-semibold cursor-pointer text-foreground/80">
                                      None
                                    </Label>
                                  </div>
                                </RadioGroup>
                              ...
                              {group.isExpanded && (
                                <div className="space-y-1 pl-1">
                                  {group.participants.map(p => {
                                    ...
                                    const isTrained = modelTrainedPaths.has(p.path)
                                    return (
                                      <div
                                        key={p.path}
                                        className={cn(
                                          "flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer transition-colors text-xs",
                                          isSelected ? "bg-primary/5 hover:bg-primary/10" : "hover:bg-surface-3/40"
                                        )}
                                      ...
                                      >
                                        ...
                                        <div
                                          className={cn(
                                            ...
                                            isTrained ? "bg-emerald-500" : "bg-amber-500"
                                          )}
                                          title={isTrained ? "Included in trained models" : "Pending (not trained)"}
                                        />
                  ...
                  {selectedProjects.length > 0 && (
                    <div className="flex items-center justify-between text-xs font-semibold px-1 pt-1">
                      <span className="text-primary">{selectedProjects.length} participant(s) selected</span>
                      <span className="text-muted-foreground/50">{projectGroups.reduce((acc, g) => acc + g.participants.length, 0)} total</span>
                    </div>
                  )}
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="models" className="border-b border-white/5">
                <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-white/[0.02] text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground/40 ml-auto font-mono bg-white/5 px-2 py-0.5 rounded-full">{brainModels.length}</span>
                  </div>
                </AccordionTrigger>
                  ...
                  {brainModels.length > 0 ? (
                    <div className="space-y-2">
                      {brainModels.map((m: any, i: number) => {
                        ...
                        const bestAcc = m.metadata?.best_val_acc ?? m.metadata?.history?.acc?.slice(-1)[0] ?? null
                        const epochsTrained = m.metadata?.epochs_completed ?? m.metadata?.history?.acc?.length ?? null
                          ...
                          >
                            ...
                            <div className="p-3 space-y-2 relative group/card">
                              ...
                              <div className="flex items-start gap-2.5">
                                ...
                                <button
                                  ...
                                  title={isModelActive ? "Active inference model" : "Set as active inference model"}
                                >
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5">
                                    <Badge variant="outline" className="text-[10px] uppercase font-bold bg-white/5 border-0 px-1.5 py-0.5 rounded">
                                      {m.architecture}
                                    </Badge>
                                    {isModelActive && (
                                  ...
                                  <h4 className="text-xs font-bold text-foreground/90 truncate mt-1" title={m.variant || m.architecture}>
                                    {m.variant || m.architecture}
                                  </h4>
                                </div>

                                <div className="text-right shrink-0">
                                  <span className="text-xs font-semibold text-muted-foreground/60 block font-mono">{m.size_mb} MB</span>
                                </div>
                              </div>
                              ...
                              {(bestAcc !== null || epochsTrained !== null) && (
                                <div className="flex items-center gap-4 text-xs bg-surface-3/30 px-2 py-1 rounded-md">
                                  {bestAcc !== null && (
                                    <span>
                                      Acc: <span className="font-bold text-amber-400">{(bestAcc * 100).toFixed(1)}%</span>
                                    </span>
                                  )}
                                  {epochsTrained !== null && (
                                    <span className="text-muted-foreground/60">
                                      Epochs: <span className="font-semibold text-foreground/80">{epochsTrained}</span>
                                    </span>
                                  )}
                                </div>
                              )}
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {trainedProjects.length > 0 && (
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setExpandedModel(isExpanded ? null : m.path)}
                            ...
                            {isExpanded && trainedProjects.length > 0 && (
                              <div className="px-3 pb-3 border-t border-white/5 pt-2 bg-black/10 space-y-1.5 min-w-0 w-full overflow-hidden">
                                <span className="text-[10px] text-muted-foreground/60 uppercase font-bold tracking-wider block">Training Datasets</span>
                                <div className="space-y-1 max-h-24 overflow-y-auto custom-scrollbar min-w-0 w-full">
                                  {trainedProjects.map((p, pi) => (
                                    <div key={pi} className="relative w-full min-w-0 overflow-hidden pr-6 group/path" title={p}>
                    ...
                    <div className="mt-3 pt-3 border-t border-white/5 space-y-2">
                      ...
                      {brainHistory.multimodal && (
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-muted-foreground">Multi: <span className="text-foreground font-bold">{brainHistory.multimodal.epochs} epochs</span>, best accuracy: <span className="text-foreground font-bold">{brainHistory.multimodal.best_acc != null ? (brainHistory.multimodal.best_acc * 100).toFixed(1) + '%' : '-'}</span></span>
                        </div>
                      )}
                    </div>
                  )}
          ...
          <div className="p-4 border-t border-white/5 bg-surface-2/30 shrink-0">
            {!brainTraining ? (
              ...
              >
                <div className="w-4 h-4 rounded-full border-2 border-muted-foreground border-t-transparent animate-spin" /> Training...
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 h-full overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4 border-b border-white/5 bg-surface-2/30 shrink-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Phase:</span>
              <span className="text-xs text-primary font-bold uppercase">{brainPhase || 'Idle'}</span>
            </div>
            <div className="flex items-center gap-6">
              <div className="text-right">
                <span className="text-[10px] font-bold text-muted-foreground/60 uppercase block">Accuracy</span>
                <span className="text-base font-extrabold text-foreground tabular-nums">
                  {brainEpochData.length > 0 ? (brainEpochData[brainEpochData.length - 1].acc * 100).toFixed(1) + '%' : '--'}
                </span>
              </div>
              <div className="h-6 w-px bg-white/5" />
              <div className="text-right">
                <span className="text-[10px] font-bold text-muted-foreground/60 uppercase block">Loss</span>
                <span className="text-base font-extrabold text-foreground tabular-nums">
                  {brainEpochData.length > 0 ? brainEpochData[brainEpochData.length - 1].loss?.toFixed(4) : '--'}
                </span>
              </div>
              <div className="h-6 w-px bg-white/5" />
              <div className="text-right">
                <span className="text-[10px] font-bold text-muted-foreground/60 uppercase block">Best Val Acc</span>
                <span className="text-base font-extrabold text-amber-400 tabular-nums">
                  {bestEpoch >= 0 && brainEpochData[bestEpoch]
                    ? ((brainEpochData[bestEpoch].val_acc ?? brainEpochData[bestEpoch].acc) * 100).toFixed(1) + '%'
                    : '--'}
                </span>
              </div>
              <div className="h-6 w-px bg-white/5" />
              <div className="text-right">
                <span className="text-[10px] font-bold text-muted-foreground/60 uppercase block">Epoch</span>
                <span className="text-base font-extrabold text-foreground tabular-nums">
                  {brainEpochData.length > 0 ? `${brainEpochData.length}/${brainEpochs}` : '--'}
                </span>
              </div>
            </div>
          </div>
              ...
              {brainEpochData.length > 0 ? (
                  ...
                  <tbody>
                    {brainEpochData.map((d: any, i: number) => {
                      ...
                      return (
                        ...
                        >
                          <td className={cn("py-2.5 pl-4 pr-4 font-bold", isBest ? "text-amber-400" : "text-primary")}>{d.epoch}</td>
                          <td className="py-2.5 pr-4 text-right text-muted-foreground/80 tabular-nums">{d.loss?.toFixed(4)}</td>
                          <td className="py-2.5 pr-4 text-right text-foreground tabular-nums">{d.acc != null ? (d.acc * 100).toFixed(1) + '%' : '-'}</td>
                          <td className="py-2.5 pr-4 text-right text-muted-foreground/80 tabular-nums">{d.val_loss?.toFixed(4) || '-'}</td>
                          <td className={cn("py-2.5 pr-4 text-right tabular-nums font-bold", isBest ? "text-amber-400" : "text-foreground")}>
                            {d.val_acc != null ? (d.val_acc * 100).toFixed(1) + '%' : '-'}
                            {isBest && <span className="ml-2 text-[10px] font-bold text-amber-400 bg-amber-400/10 px-1.5 py-0.5 rounded">★ BEST</span>}
                          </td>
                        </tr>
                      )
                    })}
                ...
                <div className="h-full flex flex-col items-center justify-center gap-4 select-none">
                  ...
                  <div className="text-center space-y-1.5">
                    <p className="text-sm tracking-[0.2em] font-extrabold uppercase text-foreground/20">Training Monitor</p>
            ...
            {brainTraining && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-primary/20">
                <div
                  ...
                  style={{ width: `${Math.min(brainPhaseProgress * 100, 100)}%` }}
                />
              </div>
            )}
          </div>
          ...
          <div className="p-3 bg-surface-2 border-t border-white/5 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className={cn("w-1.5 h-1.5 rounded-full", brainTraining ? "bg-primary animate-pulse" : "bg-[#2da44e]")} />
              ...
              {selectedProjects.length > 0 && (
                <>
                  ...
                  <span className="text-xs font-bold text-muted-foreground uppercase tracking-tight">
                    {selectedProjects.length} participant{selectedProjects.length !== 1 ? 's' : ''} selected
                  </span>
                </>
              )}
            </div>
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="text-xs font-bold uppercase bg-white/5 border-0 rounded">
                {brainArchitecture}
              </Badge>
              <Badge variant="outline" className="text-xs font-bold uppercase bg-white/5 border-0 rounded">
                LR {brainLR}
        ...
        <DrawerContent className="h-full w-full max-w-md border-l border-white/10 bg-surface-2/95 backdrop-blur-xl">
          <DrawerHeader className="border-b border-white/5 pb-4">
            ...
            <DrawerDescription className="text-xs text-muted-foreground mt-1">
              Refine your distraction prediction ML models using these core dataset and training principles.
            </DrawerDescription>
          </DrawerHeader>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
            ...
            <div className="bg-surface-3/30 border border-white/5 rounded-xl p-4 space-y-2">
              ...
              <div className="text-xs text-muted-foreground/80 leading-relaxed space-y-2">
                <p>
                  <strong>LEGACY (MLP Classifier)</strong> trains strictly on 1D driver telemetry (head angles, movement speed, variance) parsed from vehicle signals (MF4). It is very fast and lightweight but lacks camera visual behavior tracking.
                </p>
                <p>
                  <strong>MULTIMODAL (CNN + LSTM)</strong> fuses 1D vehicle signals with deep visual features extracted from driver video frames (AVI). By combining signal variance with eye gaze and posture features, it is much more accurate and robust but takes longer to train.
                </p>
              </div>
            </div>

            {BEST_PRACTICES.map((bp, i) => (
              <div key={i} className="bg-surface-3/20 border border-white/5 rounded-xl p-4 hover:bg-surface-3/45 transition-colors">
                <div className="flex items-center gap-2 mb-2">
                  <bp.icon className="w-4 h-4 text-primary shrink-0" />
                  <span className="text-xs font-bold text-foreground uppercase tracking-wide">{bp.title}</span>
                </div>
                <p className="text-xs text-muted-foreground/80 leading-relaxed">{bp.text}</p>
              </div>
            ))}
          </div>
        </DrawerContent>
        ...
        <AlertDialogContent className="max-w-[340px] border border-white/10 bg-surface-2/80 backdrop-blur-xl p-6 text-center flex flex-col items-center gap-4 rounded-3xl shadow-2xl">
          ...
          <AlertDialogHeader className="items-center text-center gap-1.5">
            ...
            <AlertDialogDescription className="text-sm text-white/70 max-w-[280px]">
              This will permanently delete the saved model file. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
