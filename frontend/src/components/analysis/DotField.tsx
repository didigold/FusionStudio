...
interface DotFieldProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /** @deprecated use darkGradientFrom/lightGradientFrom instead */
  gradientFrom?: string;
  /** @deprecated use darkGradientTo/lightGradientTo instead */
  gradientTo?: string;
  glowColor?: string;
}
...
const DotField = memo(({
  ...
  propsRef.current = { dotRadius, dotSpacing, cursorRadius, cursorForce, bulgeOnly, bulgeStrength, sparkle, waveAmplitude, darkGradientFrom, darkGradientTo, lightGradientFrom, lightGradientTo, gradientFrom, gradientTo };
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      // Detect theme at runtime so color updates immediately on theme switch
      // without depending on React re-renders / memo invalidation.
      const isDarkNow = document.documentElement.classList.contains('dark');
      const fromColor = isDarkNow
        ? (p.darkGradientFrom ?? 'rgba(255, 255, 255, 0.75)')
        : (p.lightGradientFrom ?? 'rgba(80, 80, 80, 0.6)');
      const toColor = isDarkNow
        ? (p.darkGradientTo ?? 'rgba(255, 255, 255, 0.45)')
        : (p.lightGradientTo ?? 'rgba(100, 100, 100, 0.4)');
