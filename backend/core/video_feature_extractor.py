...
class VideoFeatureExtractor:
    ...
    def __init__(self, cache_dir=None, fps_sample=5, on_log=None, on_progress=None):
    ...
    def _extract_landmarks(self, frame_bgr, face_landmarker):
        ...
        ok, rot_vec, _ = _cv2.solvePnP(face_3d, face_2d, cam_mat, dist)
        h_angle, v_angle = 0.0, 0.0
        if ok:
            rmat, _ = _cv2.Rodrigues(rot_vec)
            angles, _, _, _, _, _ = _cv2.RQDecomp3x3(rmat)
    ...
    def extract_to_cache(self, video_path, case_key):
        if not video_path or not os.path.exists(video_path):
            if self.on_log:
                self.on_log(f"  Video not found for {case_key}, skipping.")
            return None
        ...
        if os.path.exists(cache_path):
            ...
            if existing.shape[1] != self.FEATURE_DIM + 1:
                if self.on_log:
                    self.on_log(f"  Cache dimension mismatch ({existing.shape[1]} vs {self.FEATURE_DIM + 1}), regenerating...")
                os.remove(cache_path)
            else:
                if self.on_log:
                    self.on_log(f"  Cache hit: {os.path.basename(cache_path)}")
                return cache_path
        ...
        if not cap.isOpened():
            if self.on_log:
                self.on_log(f"  Cannot open video: {os.path.basename(video_path)}")
            return None
        ...
        if not embeddings:
            if self.on_log:
                self.on_log(f"  No frames extracted from {os.path.basename(video_path)}")
            return None
        ...
        if self.on_log:
            self.on_log(f"  Cached {len(embeddings)} frames in {elapsed:.1f}s → {os.path.basename(cache_path)}")
        return cache_path
    ...
    def load_from_cache(self, case_key):
        ...
        if os.path.exists(cache_path):
            ...
            if data.shape[1] != self.FEATURE_DIM + 1:
                if self.on_log:
                    self.on_log(f"  Cache dimension mismatch for {case_key}, removing invalid cache")
                os.remove(cache_path)
                return None
            return data
        return None
