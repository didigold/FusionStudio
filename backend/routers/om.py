...
from backend.routers.reporting import update_excel_results
...
def build_om_report_config(file_path: str, protocol: str, metadata: dict, category_configs: dict, gauge_rules: dict, driver_marks: list, micro: dict = None, show_thresholds: bool = False) -> dict:
    import re
    import numpy as np
    from asammdf import MDF
...
@router.post("/preview")
async def om_preview(req: OMPreviewRequest):
    import traceback
    try:
        ...
        marks_dict = _load_marks_dict(source_dir, "OM")
        ...
        preview_output_path = os.path.join(temp_dir, "om_preview.png")
        ...
        logger.error(f"Error generating om preview: {e}")
        traceback.print_exc()
