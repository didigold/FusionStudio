...
v2.0: Multimodal (CNN+LSTM) + Legacy MLP support.
...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QTextEdit, QFrame, QSplitter, QGroupBox, QDoubleSpinBox, QSpinBox, QGridLayout, QFileDialog, QLineEdit, QComboBox, QScrollArea, QTabWidget)
...
from src.ui.widgets import AnimatedExpandButton, AnimatedToggle, PulsingProgressBar
import time
...
class AIBrainWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.col2_tabs.setStyleSheet("""
            QTabWidget::pane { border: none; background: transparent; }
            QTabBar::tab {
                background-color: #2b2b2b; color: #aaa; padding: 6px 12px;
                border: 1px solid #444; border-bottom: none; border-radius: 4px 4px 0 0;
                font-family: Switzer; font-size: 8pt;
            }
            QTabBar::tab:selected { background-color: #1e1e1e; color: white; border-bottom: 2px solid #F39200; }
        ...
        self.lbl_preview_title = QLabel("Select a project to preview")
        self.lbl_preview_title.setStyleSheet("color: #888; font-size: 9pt;")
        ...
        self.lbl_history.setStyleSheet("color: #888; font-size: 9pt;")
        self.lbl_history.setWordWrap(True)
        history_layout.addWidget(self.lbl_history)
        from PySide6.QtWidgets import QTableWidget, QTableWidgetItem, QHeaderView
        self.history_table = QTableWidget()
        self.history_table.setColumnCount(6)
        self.history_table.setHorizontalHeaderLabels(["Build", "Epochs", "Val Loss", "Acc", "F1", "Date"])
        self.history_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.history_table.setStyleSheet("""
            QTableWidget { background-color: #1e1e1e; color: #ccc; border: none; gridline-color: #333; }
            QHeaderView::section { background-color: #2b2b2b; color: #aaa; border: none; padding: 4px; font-size: 9pt; }
        """)
        ...
        lbl_export = QLabel("Export model to ONNX format for deployment.")
        lbl_export.setStyleSheet("color: #aaa; font-size: 9pt;")
        ...
        self.btn_export_onnx.setStyleSheet("""
            QPushButton {
                background-color: #2b2b2b; color: #ccc; padding: 12px; border: 1px solid #555;
        ...
        export_layout.addWidget(self.lbl_export_result)
        ...
        self.tab_widget.setStyleSheet("""
            QTabWidget::pane { border: none; background: transparent; }
            QTabBar::tab {
                background-color: #2b2b2b; color: #aaa; padding: 8px 16px;
                border: 1px solid #444; border-bottom: none; border-radius: 6px 6px 0 0;
                font-family: Switzer; font-size: 9pt;
            }
            QTabBar::tab:selected { background-color: #1e1e1e; color: white; border-bottom: 2px solid #F39200; }
            QTabBar::tab:hover { background-color: #333; }
        """)
        ...
        self.plot_cpu.setMinimumHeight(80)
        self.plot_cpu.setBackground('#1e1e1e')
        self.plot_cpu.setLabel('left', 'CPU %')
        self.plot_cpu.getAxis('bottom').setTicks([])
        self.plot_cpu.getAxis('bottom').setStyle(showValues=False)
        self.cpu_curve = self.plot_cpu.plot(pen=pg.mkPen('#3498db', width=2))
        self.cpu_data = []

        self.plot_ram = pg.PlotWidget()
        self.plot_ram.setMinimumHeight(80)
        self.plot_ram.setBackground('#1e1e1e')
        self.plot_ram.setLabel('left', 'RAM MB')
        self.plot_ram.getAxis('bottom').setTicks([])
        self.plot_ram.getAxis('bottom').setStyle(showValues=False)
        self.ram_curve = self.plot_ram.plot(pen=pg.mkPen('#2ecc71', width=2))
        self.ram_data = []

        self.plot_gpu = pg.PlotWidget()
        self.plot_gpu.setMinimumHeight(80)
        self.plot_gpu.setBackground('#1e1e1e')
        self.plot_gpu.setLabel('left', 'GPU %')
        self.plot_gpu.getAxis('bottom').setTicks([])
        self.plot_gpu.getAxis('bottom').setStyle(showValues=False)
        self.gpu_curve = self.plot_gpu.plot(pen=pg.mkPen('#f39c12', width=2))
        self.gpu_data = []

        system_layout.addWidget(self.plot_cpu)
        system_layout.addWidget(self.plot_ram)
        system_layout.addWidget(self.plot_gpu)
        system_layout.addStretch()

        self.lbl_sys_info = QLabel("System monitoring: waiting...")
        self.lbl_sys_info.setStyleSheet("color: #888; font-size: 9pt; background: transparent; border: none;")
        system_layout.addWidget(self.lbl_sys_info)
    ...
    def _add_project_to_tree(self, path, is_trained=False):
        ...
        if os.path.exists(marks_file):
            ...
            btn_file.setStyleSheet("""
                QPushButton {
                    background: transparent; color: #aaa; text-align: left; padding: 3px 6px; border: none; border-radius: 4px;
        ...
        if available_cams or avi_files:
            vid_widget = QWidget()
            vid_hlayout = QHBoxLayout(vid_widget)
            vid_hlayout.setContentsMargins(0, 0, 0, 0)
            vid_hlayout.setSpacing(4)
            vid_icon = QLabel()
            vid_icon.setPixmap(QIcon(resource_path("assets/icons/videocam_16dp_FFFFFF_FILL0_wght400_GRAD0_opsz20.png")).pixmap(14, 14) if os.path.exists(resource_path("assets/icons/videocam_16dp_FFFFFF_FILL0_wght400_GRAD0_opsz20.png")) else QIcon().pixmap(14, 14))
            vid_hlayout.addWidget(vid_icon)
            cam_combo = QComboBox()
            cam_combo.addItem("Auto")
            for cam in available_cams:
                cam_combo.addItem(cam.lstrip("_").capitalize())
            cam_combo.setFixedHeight(24)
            cam_combo.setStyleSheet(self.COMBO_STYLE)
            vid_hlayout.addWidget(cam_combo)
            vid_count = QLabel(f"({len(avi_files)})")
            vid_count.setStyleSheet("color: #777; font-size: 9pt; background: transparent; border: none;")
            vid_hlayout.addWidget(vid_count)
            cam_combo.currentTextChanged.connect(lambda text, p=norm_path: self._on_camera_change(p, text))
            data_hlayout.addWidget(vid_widget)
        
        if mf4_files:
            mf4_icon = QLabel()
            mf4_icon.setPixmap(QIcon(resource_path("assets/icons/deployed_code_16dp_FFFFFF_FILL0_wght400_GRAD0_opsz20.png")).pixmap(14, 14) if os.path.exists(resource_path("assets/icons/deployed_code_16dp_FFFFFF_FILL0_wght400_GRAD0_opsz20.png")) else QIcon().pixmap(14, 14))
            data_hlayout.addWidget(mf4_icon)
            mf4_count = QLabel(f"({len(mf4_files)})")
            mf4_count.setStyleSheet("color: #777; font-size: 9pt; background: transparent; border: none;")
            data_hlayout.addWidget(mf4_count)
    ...
    def _populate_build_history(self):
        ...
        for row, (build_name, meta) in enumerate(builds):
            ...
            self.history_table.setItem(row, 5, QTableWidgetItem(f"{n_projects} proj"))
    ...
    def _on_system_stats(self, data):
        ...
        if len(self.ram_data) > max_points:
            self.ram_data = self.ram_data[-max_points:]
        if len(self.gpu_data) > max_points:
            self.gpu_data = self.gpu_data[-max_points:]
        ...
        if predictions:
            for i in range(0, len(predictions), 2):
                if i+1 < len(predictions):
                    lr = pg.LinearRegionItem([predictions[i], predictions[i+1]], movable=False, brush=pg.mkBrush(0, 255, 0, 80))
                    self.preview_plot.addItem(lr)
