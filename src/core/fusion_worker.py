...
from src.core.utils import clone_file_metadata, resource_path
from src.ui.styles import IDIADA_ORANGE

class ParticipantScanner(QThread):
    ...
    def run(self):
        results = []
        try:
            # Detectar carpetas de participantes (Ej: P01, P02...)
            parts = [d for d in os.listdir(self.source_dir) if os.path.isdir(os.path.join(self.source_dir, d)) and re.match(r"^[A-Z][0-9]{2}$", d)]
            parts.sort()
            ...
            for p in parts:
                ...
                # --- SCANNER MEJORADO: Buscar recursivamente satélites y masters ---
                masters = []
                satellites = []
                
                # Recorrer subcarpetas para encontrar todos los MF4
                for root, _, files in os.walk(p_path):
                    if "_FUSION_RESULTS" in root or "_TEMP" in root: continue
                ...
                # --- ESTADO Y CONTEO (Status Column) ---
                count_total_files = len(satellites) 
                
                # Contar videos AVI recursivamente
                count_total_vids = 0
                for root, _, files in os.walk(p_path):
                    if "_FUSION_RESULTS" in root or "_TEMP" in root: continue
                    for f in files:
                ...
                # Contar Procesados en carpeta Output
                count_done_files = 0
                count_done_vids = 0
...
class PreviewSignalsWorker(QThread):
    finished = Signal(list)
    error = Signal(str)
            ...
            with MDF(self.file_path) as mdf:
                for g_idx, group in enumerate(mdf.groups):
                    ...
                    for c_idx, ch in enumerate(group.channels):
                        name = ch.name
                        count = "N/A"
                        try: count = group.channel_group.cycles_nr
                        except: pass
...
class FusionWorkerSignals(QObject):
    log = Signal(str)
    progress = Signal(float)
    participant_progress = Signal(str, int) 
    finished = Signal()
    error = Signal(str)
    participant_status = Signal(str, str)
    cleaning_mem = Signal(bool) 

class FusionWorker(QThread):
    def __init__(self, source_dir, participants_to_process, signal_whitelist=None, copy_videos=False, overwrite_mode=False):
        super().__init__()
        self.source_dir = source_dir
        ...
        self.signals = FusionWorkerSignals()
    ...
    def pause(self):
        self.is_paused = True
        self.signals.log.emit(f"[{self._ts()}] ⏸️ PROCESS PAUSED")

    def resume(self):
        self.is_paused = False
        self.signals.log.emit(f"[{self._ts()}] ▶️ PROCESS RESUMED")

    def stop(self):
        self.is_running = False
        self.is_paused = False 
        self.signals.log.emit(f"[{self._ts()}] 🛑 STOP REQUESTED...")
    ...
    def run(self):
        self.signals.log.emit(f"[{self._ts()}] --- 🚀 STARTING BATCH PROCESS (v26.10) ---")
        if self.overwrite_mode: self.signals.log.emit(f"[{self._ts()}] --- ⚠️ OVERWRITE MODE: ON ---")
        if self.signal_whitelist: self.signals.log.emit(f"[{self._ts()}] --- Filter: ACTIVE ({len(self.signal_whitelist)} signals) ---")
        else: self.signals.log.emit(f"[{self._ts()}] --- Filter: OFF (All Signals) ---")
        vid_msg = "ON" if self.copy_videos else "OFF"
        self.signals.log.emit(f"[{self._ts()}] --- Video Copy: {vid_msg} ---")
        ...
        try:
            ...
            for idx, p_name in enumerate(self.participants_to_process):
                ...
                self.signals.participant_progress.emit(p_name, 0)
                
                def update_sub_progress(val_in_participant):
                    self.signals.participant_progress.emit(p_name, int(val_in_participant * 100))
                ...
                try:
                    ...
                    if self.is_running:
                        self.signals.participant_status.emit(p_name, "Done")
                        self.signals.participant_progress.emit(p_name, 100) 
                except Exception as ex_part:
                     self.signals.log.emit(f"[{self._ts()}] ❌ CRITICAL ERROR in {p_name}: {ex_part}")
                     self.signals.participant_status.emit(p_name, "Error")
                
                if idx < total_parts - 1 and self.is_running:
                    self.signals.log.emit(f"[{self._ts()}] 🧹 Cleaning RAM memory (5s wait)...")
                    self.signals.cleaning_mem.emit(True) 
                    gc.collect()
                    for _ in range(50): 
                        if not self.is_running: break
                        time.sleep(0.1)
                    self.signals.cleaning_mem.emit(False) 

            self.signals.progress.emit(1.0)
            if not self.is_running: self.signals.log.emit(f"[{self._ts()}] --- 🛑 BATCH STOPPED ---")
            else: self.signals.log.emit(f"[{self._ts()}] --- ✅ BATCH COMPLETED ---")
    ...
    def obtener_maestros(self, carpeta):
        ...
        for f in all_files:
            ...
            try:
                ...
                score = 0
                if f.upper().startswith(p_name.upper()): score += 100
    ...
    def buscar_satelites(self, carpeta):
        ...
        for root, _, files in os.walk(carpeta):
            if "_TEMP" in root or "_FUSION_RESULTS" in root: continue
            if root == carpeta: continue
            for f in files:
                if f.lower().endswith(".mf4"): mapa[f] = os.path.join(root, f)
        return mapa
    def buscar_videos(self, carpeta):
        ...
        for root, _, files in os.walk(carpeta):
            if "_TEMP" in root or "_FUSION_RESULTS" in root: continue
            for f in files:
                if f.lower().endswith(".avi"): mapa.append(os.path.join(root, f))
        return mapa
    def get_names(self, tipo, num, test, ob):
        carpeta_tipo = {1: "Custom", 2: "Distractions", 3: "Fatigue", 4: "Occlusions", 5: "Behaviour", 6: "Behaviour", 8: "ADDW low speed", 10: "ADDW high speed"}.get(tipo, "Unknown")
        prefijo = "X"
        if tipo == 2: prefijo = "D"
        elif tipo == 3: prefijo = "F"
        elif tipo in [8, 10]: prefijo = "ADDW"
        elif tipo in [4, 5]: prefijo = "D" if num <= 42 else "F"
        nombre = ""
        if ob == 0: nombre = f"{prefijo}{num}_{test}.mf4"
        else:
    ...
    def procesar_participante(self, ruta_p, ruta_salida, progress_cb):
        ...
        self.signals.log.emit(f"[{self._ts()}] 📂 Participant: {nombre_p}")
        
        maestros = self.obtener_maestros(ruta_p)
        if not maestros:
            self.signals.log.emit(f"[{self._ts()}]    ⚠️ No valid Master files found.")
            return

        total_maestros = len(maestros)
        self.signals.log.emit(f"[{self._ts()}]    🔎 Found {total_maestros} potential Master file(s).")
        ...
        for m_idx, maestro in enumerate(maestros):
            ...
            self.signals.log.emit(f"[{self._ts()}]    💿 Processing Master {m_idx+1}/{total_maestros}: {m_name}")
            
            base_m = m_idx / total_maestros
            step_m = 1.0 / total_maestros
            
            def update_phase_1(val): 
                local_p = base_m + (step_m * 0.4 * val)
                if progress_cb: progress_cb(local_p)
            
            def update_phase_2(val): 
                local_p = base_m + (step_m * 0.4) + (step_m * 0.6 * val)
                if progress_cb: progress_cb(local_p)

            cortes = self.fase_1_auditoria_y_cortes(maestro, dir_temp, ruta_p, dir_final, update_phase_1)
            
            if not cortes:
                self.signals.log.emit(f"[{self._ts()}]       (No new cuts generated from this master)")
                update_phase_1(1.0) 
            else:
                n_ok = self.fase_2_fusionar(cortes, ruta_p, dir_final, update_phase_2)
                self.signals.log.emit(f"[{self._ts()}]       ✅ Fused {n_ok} files from {m_name}.")

            gc.collect()

        try: shutil.rmtree(dir_temp)
        except: pass
        
        if self.copy_videos and self.is_running: self.fase_3_copiar_videos(ruta_p, dir_final)
        ...
        try:
            ...
            with MDF(maestro) as mdf:
                if "Distraction_type" not in mdf:
                    self.signals.log.emit(f"[{self._ts()}]       ⚠️ Skipped: Missing 'Distraction_type'. Likely a satellite.")
                    return []
            ...
            if fines and fines[0] < inicios[0]: fines.pop(0)
            ...
            for i, (ini, fin) in enumerate(zip(inicios, fines)):
                ...
                if fname not in mapa_satelites: continue
                ...
                if not self.overwrite_mode and os.path.exists(path_final):
                    ...
                    if size_final > size_original + 500: needs_action = False
                
                if not needs_action: continue

                if os.path.exists(path_temp) and os.path.getsize(path_temp) > 1024: generados.append(path_temp)
                else: to_cut.append({"t_start": timestamps[ini], "t_stop": timestamps[fin], "path": path_temp, "dir": os.path.join(dir_temp, folder), "name": fname})
            
            if not to_cut: return generados
            
            total_cuts = len(to_cut)
            self.signals.log.emit(f"[{self._ts()}]       ✂️ Found {total_cuts} segments to cut...")
            ...
            if self.signal_whitelist:
                ...
                try:
                    ...
                    self.signals.log.emit(f"[{self._ts()}]       ⚠️ Audit Error: {e_audit}")
                ...
                if missing_signals:
                    ...
                    self.signals.log.emit(f"[{self._ts()}]       {msg}")

                if not found_signals:
                     self.signals.log.emit(f"[{self._ts()}]       ⚠️ ALL selected signals missing! Fallback to KEEP ALL signals.")
                     final_whitelist = None
                else:
                     final_whitelist = list(set(found_signals))
                     self.signals.log.emit(f"[{self._ts()}]       ℹ️ Using {len(final_whitelist)}/{len(self.signal_whitelist)} signals (Adapted to file structure).")
            ...
            with MDF(maestro) as mdf:
                for idx, item in enumerate(to_cut):
                    ...
                    if progress_cb: progress_cb(progreso_local)
                    ...
                    try:
                        if final_whitelist:
                            ...
                            for name, g_idx, c_idx in final_whitelist:
                                if g_idx not in groups_map: groups_map[g_idx] = []
                            ...
                            for g_idx in sorted(groups_map.keys()):
                                ...
                                try:
                                    ...
                                    del temp_res
                                    if 'temp_mdf' in locals(): del temp_mdf
                                    del temp_cut
                                    del signals_to_append
                                    gc.collect()
                                except Exception as e_sel:
                                    self.signals.log.emit(f"[{self._ts()}]       ⚠️ Select Error: {e_sel}")  
                        else:
                    ...
                    except Exception as e: self.signals.log.emit(f"[{self._ts()}]       ⚠️ Cut Error {item['name']}: {e}")
                    finally:
                        if final_mdf: del final_mdf
                        if idx % 5 == 0: gc.collect() 
            return generados
        except Exception as e:
            self.signals.error.emit(f"Phase 1 Error: {e}")
            return []
                        ...
                        with MDF(ruta_corte) as mdf_c:
                            for i, grp in enumerate(mdf_c.groups):
                                ...
                                for ch_idx, ch in enumerate(grp.channels):
                                    ...
                                    if len(s.timestamps) > 0: s.timestamps -= s.timestamps[0]
                                    sigs.append(s)
                                if sigs: mdf_s.append(sigs)
                                del sigs
                            mdf_s.save(ruta_dest, overwrite=True)
                    clone_file_metadata(original_sat_path, ruta_dest)
                    count += 1
                except Exception as e: self.signals.log.emit(f"[{self._ts()}]       ❌ Fusion Error: {e}")
            gc.collect()
        return count

    def fase_3_copiar_videos(self, ruta_origen, dir_final_base):
        self.signals.log.emit(f"[{self._ts()}]    🎥 Copying videos...")
        ...
        for vid_path in videos:
            ...
            if self.overwrite_mode or not os.path.exists(dest):
                ...
                try:
                    ...
                    count += 1
                except Exception as e: self.signals.log.emit(f"[{self._ts()}]       ❌ Video Error: {e}")
        if count > 0: self.signals.log.emit(f"[{self._ts()}]    🎥 {count} videos copied.")
