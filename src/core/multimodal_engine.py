...
class MultimodalTrainer(QObject):
    log = Signal(str)
    epoch_progress = Signal(int, float, float, float, float)
    ...
    def load_model(self, model_path):
        ...
        if os.path.exists(model_path):
            try:
                ...
                self.log.emit(f"Multimodal model loaded from {os.path.basename(os.path.dirname(model_path))}")
                return True
            except Exception as e:
                self.log.emit(f"Error loading multimodal model: {e}")
                return False
        return False
    ...
    def train(self, signal_windows, video_windows, labels, project_ids, model_name, epochs, lr, project_paths, early_stop_patience=15):
        ...
        try:
            self.log.emit("Preparing multimodal dataset...")
            ...
            self.log.emit(f"Train: {len(train_sig)} windows from {len(train_projects)} projects")
            self.log.emit(f"Val: {len(val_sig)} windows from {len(val_projects)} projects")
            ...
            self.log.emit(f"Initializing MultimodalDetector (signal_dim={sig_dim}, video_dim={vid_dim})...")
            ...
            self.log.emit(f"Training multimodal model '{model_name}' for {epochs} epochs...")
            ...
            for epoch in range(1, epochs + 1):
                ...
                if val_loader is not None:
                    ...
                    if val_loss < best_val_loss:
                        ...
                        if patience_counter >= early_stop_patience:
                            self.log.emit(f"Early stopping at epoch {epoch} (no improvement for {early_stop_patience} epochs)")
                            break
                ...
                self.epoch_progress.emit(epoch, train_loss, train_acc, train_f1, val_loss)

            if best_state is not None:
                self.model.load_state_dict(best_state)
                self.log.emit(f"Restored best model (val_loss={best_val_loss:.4f})")
            ...
            import traceback
            self.log.emit(f"Multimodal training failed: {e}")
            traceback.print_exc()
